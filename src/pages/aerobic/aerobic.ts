    ...
    dbs.collection("Aerobic").get()
      .then(function (querySnapshot) {
        ...
        querySnapshot.forEach(function (doc) {
          ...
          getCourse[i] = doc.id;
          //----------------------
          dbs.collection("Aerobic").doc(getCourse[i]).get().then((doc) => {
            if (doc.exists) {
      ...
      dataRef.once('value', dataSnapshot => {
        try {
          ...
          let alert = this.alertCtrl.create({
            ...
            subTitle: "ท่านได้ทำการลงทะเบียนคอร์ส 'แอโรบิค' อยู่แล้ว",
            buttons: ['ตกลง']
          });
          alert.present();
          ...
          let dbs = firebase.firestore();
          dbs.collection("Aerobic").doc(this.radio1).get().then((doc) => {
            if (doc.exists) {
              var uid = firebase.auth().currentUser.uid;
              if (doc.data().registered < doc.data().get) {
                const confirm = this.alertCtrl.create({
                  title: 'ยืนยันการสมัคร?',
                  message: "คุณต้องการที่จะสมัครคอร์ส 'แอโรบิค' ?",
                  buttons: [
                    {
                      text: 'ไม่ตกลง',
                      handler: () => {
                    ...
                    },
                    {
                      ...
                      handler: () => {
                        ...
                        dbs.collection('Aerobic').doc(this.radio1).update({
                          registered: reg
                        })
                        firebase.database().ref('/users/' + this.sid + '/' + uid + '/courseReg/Aerobic').set({
